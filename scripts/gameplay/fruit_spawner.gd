...
var palette: Array[Color] = [
	Color("ff5b5b"), Color("ffb347"), Color("f6df5f"),
	Color("78df72"), Color("b46cff"), Color("ff70b7")
]
...
func _spawn_item(kind: Sliceable.Kind, index: int, count: int) -> void:
	...
	var color := palette[random.randi_range(0, palette.size() - 1)]
	add_child(item)
	item.configure(kind, color, Vector3(horizontal, upward, 0.0), camera)
